 ...
 */
class DetalhePedido extends TPage
{
    protected $form; 
    protected $dt_venda;
    ...
    function __construct($param)
    {
        ...
        $label_data     = new TLabel('Data');
        $label_cliente  = new TLabel('Cliente');
        
        $this->form->addFields( [new TLabel('ID')], [$id] );
        $this->form->addFields( [$label_data], [$data] );
        $this->form->addFields( [$label_cliente], [$cliente_id, $cliente_name] );
        $this->form->addFields( [new TLabel('Obs')], [$obs] );
        
        $label_date->setFontColor('#FF0000');
        
        // create detail fields
        $product_id = new TDBUniqueSearch('produto_id[]', 'cadastro', 'produto', 'id', 'nome');
        ...
        $product_price = new TEntry('preco_venda[]');
        ...
        $product_amount = new TEntry('qtd_produto[]');
        ...
        $product_total = new TEntry('valor_total[]');
        ...
        $this->form->addField($produto_qtd);
        $this->form->addField($produto_total);
        
        // detail
        $this->product_list = new TFieldList;
        $this->product_list->addField( '<b>Produto</b>',     $produto_id,     ['width' => '40%']);
        $this->product_list->addField( '<b>Preço</b>',       $preco,  ['width' => '20%']);
        $this->product_list->addField( '<b>Quantidade</b>',  $qtd, ['width' => '20%']);
        $this->product_list->addField( '<b>Total</b>',       $produto_total,  ['width' => '20%', 'sum' => true]);
    ...
    function onEdit($param)
    {
        try
        {
            ...
            if (isset($param['key']))
            {
                ...
                $produtos = new Produto($key);
                $this->form->setData($produtos);
                
                $produtos_item = SaleItem::where('produto_id', '=', $produtos->id)->load();
                
                $this->product_list->addHeader();
                if ($produtos_item)
                {
                    foreach($produtos_item  as $item )
                    {
                        $item->produto_preco  = $item->preco;
                        $item->produto_qtd = $item->qtd;
                        $item->produto_total  = $item->produto_preco * $item->qtd;
